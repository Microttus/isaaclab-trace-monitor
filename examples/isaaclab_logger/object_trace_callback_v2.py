...
The callback writes compact, MATLAB-friendly, wide CSV files containing one
...
Directory layout below ``log_dir / subdir``::
    ...
    archive/
        env_000/
            episode_000000.csv
            episode_000025.csv
            ... every Nth episode, never pruned (only when enabled)
...
Each requested contact sensor contributes the strongest filtered contact vector
per sample::

    <sensor>_fx_N, <sensor>_fy_N, <sensor>_fz_N, <sensor>_force_N

When enabled, the configured joint state entity contributes actual joint state
columns, independent of the SB3 action vector::

    <prefix>_<label>_pos_rad, <prefix>_<label>_vel_rad_s
...
class _TerminalSnapshot:
    """State and termination data captured before Isaac Lab resets an env."""
...
class ObjectTraceCallback(BaseCallback):
    ...
    contact_sensors:
        Optional mapping from a CSV alias to an Isaac Lab scene contact-sensor
        name. Each sensor logs the strongest filtered contact vector and its
    ...
    joint_state_entity:
        Optional articulation scene entity whose actual joint positions and
        velocities should be logged. If enabled without explicit indices or
        labels, the final two joints are logged as ``left`` and ``right``.
    joint_state_indices:
        Optional joint indices for ``joint_state_entity``. Negative indices are
        resolved from the end of the articulation joint state.
    joint_state_labels:
        Column labels corresponding to ``joint_state_indices``.
    joint_state_prefix:
        Prefix used for the actual joint-state columns.
    keep_last_episodes:
    ...
    archive_every_episodes:
        Also write every Nth completed episode per traced environment to
        ``archive/``, which ``keep_last_episodes`` never prunes. This preserves
        a sparse history of the whole run. ``0`` disables archiving.
    ...
    def __init__(
        ...
        contact_sensors: Mapping[str, str] | None = None,
        log_orientation: bool = True,
        include_actions: bool = False,
        keep_last_episodes: int = 20,
        archive_every_episodes: int = 0,
        ...
        verbose: int = 0,
        joint_state_entity: str | None = None,
        joint_state_indices: Sequence[int] | None = None,
        joint_state_labels: Sequence[str] | None = None,
        joint_state_prefix: str = "gripper",
    ) -> None:
        ...
        if archive_every_episodes < 0:
            raise ValueError("archive_every_episodes must be >= 0")
        ...
        if joint_state_entity is None and (
            joint_state_indices is not None or joint_state_labels is not None
        ):
            raise ValueError(
                "joint_state_indices and joint_state_labels require joint_state_entity"
            )
        if joint_state_entity is not None and not str(joint_state_entity).strip():
            raise ValueError("joint_state_entity must be a non-empty scene entity name")
        joint_state_prefix = str(joint_state_prefix)
        if not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", joint_state_prefix):
            raise ValueError(
                "joint_state_prefix must contain only letters, numbers, and underscores "
        ...
        self.archive_dir = self.trace_dir / "archive"
        ...
        self.contact_sensors = {
            str(alias): str(sensor_name)
            for alias, sensor_name in (contact_sensors or {}).items()
        }
        self._contact_force_prefixes = {
            alias: f"contact_{prefix}"
            for alias, prefix in self._make_unique_prefixes(tuple(self.contact_sensors)).items()
        }
        ...
        self.archive_every_episodes = int(archive_every_episodes)
        ...
        self.joint_state_entity = (
            None if joint_state_entity is None else str(joint_state_entity)
        )
        if self.joint_state_entity is None:
            self.joint_state_indices: tuple[int, ...] = ()
            self.joint_state_labels: tuple[str, ...] = ()
        else:
            self.joint_state_indices = tuple(
                int(index)
                for index in (
                    joint_state_indices if joint_state_indices is not None else (-2, -1)
                )
            )
            self.joint_state_labels = tuple(
                str(label)
                for label in (
                    joint_state_labels if joint_state_labels is not None else ("left", "right")
                )
            )
            if not self.joint_state_indices:
                raise ValueError("joint_state_indices must contain at least one joint index")
            if len(self.joint_state_indices) != len(self.joint_state_labels):
                raise ValueError("joint_state_indices and joint_state_labels must have equal lengths")
            if len(set(self.joint_state_labels)) != len(self.joint_state_labels):
                raise ValueError("joint_state_labels must be unique")
            if any(
                not re.fullmatch(r"[A-Za-z_][A-Za-z0-9_]*", label)
                for label in self.joint_state_labels
            ):
                raise ValueError(
                    "joint_state_labels must contain only letters, numbers, and underscores "
                    "and must not start with a number"
                )
        self.joint_state_prefix = joint_state_prefix
        self._resolved_joint_state_indices: tuple[int, ...] = ()
    ...
    def _on_training_start(self) -> None:
        ...
        available_sensors = set(getattr(self._scene, "sensors", {}).keys())
        missing_sensors = [
            sensor_name
            for sensor_name in self.contact_sensors.values()
            if sensor_name not in available_sensors
        ]
        if missing_sensors:
            raise KeyError(
                f"ObjectTraceCallback could not find contact sensor(s): {missing_sensors}. "
                f"Available sensors include: {sorted(available_sensors)}"
            )

        if self.joint_state_entity is not None:
            if self.joint_state_entity not in available:
                raise KeyError(
                    f"ObjectTraceCallback could not find joint-state scene entity "
                    f"{self.joint_state_entity!r}. Available scene keys include: {sorted(available)}"
                )
            self._resolve_joint_state_indices()
        ...
        for env_id in self.env_ids:
            self._append_sample(
                ...
                contact_forces=self._read_contact_forces((env_id,)).get(env_id, {}),
                joint_states=self._read_joint_states((env_id,)).get(env_id, {}),
                episode_step=0,
                reward=0.0,
                action=None,
                terminal=False,
        ...
        if self.verbose:
            print(
                ...
                f"contact_sensors={self.contact_sensors}, "
    ...
    def _on_step(self) -> bool:
        ...
        for env_id in done_env_ids:
            ...
            if terminal_snapshot is not None:
                self._append_sample(
                    ...
                    contact_forces=terminal_snapshot.contact_forces,
                    joint_states=terminal_snapshot.joint_states,
        ...
        if self.n_calls % self.sample_interval == 0:
            ...
            if active_env_ids:
                ...
                contact_forces_by_env = self._read_contact_forces(active_env_ids)
                joint_states_by_env = self._read_joint_states(active_env_ids)
                for env_id in active_env_ids:
                    ...
                    self._append_sample(
                        ...
                        contact_forces=contact_forces_by_env.get(env_id, {}),
                        joint_states=joint_states_by_env.get(env_id, {}),
    ...
    def _capture_terminal_before_reset(self, env_ids: Any) -> None:
        ...
        contact_forces_by_env = self._read_contact_forces(traced_ids)
        joint_states_by_env = self._read_joint_states(traced_ids)
    ...
    def _append_sample(
        ...
        *,
        env_id: int,
        poses: dict[str, np.ndarray],
        contact_forces: dict[str, np.ndarray] | None = None,
        ...
        for alias, prefix in self._contact_force_prefixes.items():
            values = np.asarray((contact_forces or {}).get(alias, np.full(4, np.nan)), dtype=float).reshape(-1)
            if values.size < 4:
                padded = np.full(4, np.nan, dtype=float)
                padded[: values.size] = values
                values = padded
            row[f"{prefix}_fx_N"] = float(values[0])
            row[f"{prefix}_fy_N"] = float(values[1])
            row[f"{prefix}_fz_N"] = float(values[2])
            row[f"{prefix}_force_N"] = float(values[3])

        for label in self.joint_state_labels:
            values = np.asarray(
                (joint_states or {}).get(label, np.full(2, np.nan)),
                dtype=float,
            ).reshape(-1)
            if values.size < 2:
                padded = np.full(2, np.nan, dtype=float)
                padded[: values.size] = values
                values = padded
            prefix = f"{self.joint_state_prefix}_{label}"
            row[f"{prefix}_pos_rad"] = float(values[0])
            row[f"{prefix}_vel_rad_s"] = float(values[1])
    ...
    def _read_contact_forces(self, env_ids: Iterable[int]) -> dict[int, dict[str, np.ndarray]]:
        """Read the strongest filtered contact vector for each requested sensor."""
        selected_ids = tuple(int(env_id) for env_id in env_ids)
        forces_by_env: dict[int, dict[str, np.ndarray]] = {
            env_id: {} for env_id in selected_ids
        }
        if not selected_ids or not self.contact_sensors:
            return forces_by_env
        if self._scene is None:
            raise RuntimeError("Scene is not initialized")

        ids_tensor_cache: torch.Tensor | None = None
        for alias, sensor_name in self.contact_sensors.items():
            sensor = self._scene.sensors[sensor_name]
            forces = getattr(getattr(sensor, "data", None), "force_matrix_w", None)
            if forces is None:
                raise RuntimeError(
                    f"Contact sensor '{sensor_name}' does not expose force_matrix_w."
                )
            if not isinstance(forces, torch.Tensor) or forces.ndim < 2 or forces.shape[-1] != 3:
                raise ValueError(
                    f"Expected contact sensor '{sensor_name}' force_matrix_w with final dimension 3, "
                    f"got {type(forces)!r} shape={getattr(forces, 'shape', None)}"
                )

            flattened = forces.reshape(forces.shape[0], -1, 3)
            magnitudes = torch.linalg.vector_norm(flattened, dim=-1)
            strongest_index = magnitudes.argmax(dim=1)
            env_index = torch.arange(forces.shape[0], device=forces.device)
            strongest = flattened[env_index, strongest_index]
            strongest_magnitude = magnitudes[env_index, strongest_index].unsqueeze(-1)
            values = torch.cat((strongest, strongest_magnitude), dim=-1)

            if ids_tensor_cache is None or ids_tensor_cache.device != values.device:
                ids_tensor_cache = values.new_tensor(selected_ids, dtype=torch.long)
            selected = values.index_select(0, ids_tensor_cache).detach().cpu().numpy()
            for local_idx, env_id in enumerate(selected_ids):
                forces_by_env[env_id][alias] = selected[local_idx]

        return forces_by_env

    def _resolve_joint_state_indices(self) -> None:
        if self._scene is None or self.joint_state_entity is None:
            return

        data = getattr(self._scene[self.joint_state_entity], "data", None)
        ...
        if not isinstance(joint_pos, torch.Tensor) or joint_pos.ndim != 2:
            raise ValueError(
                f"Joint-state entity '{self.joint_state_entity}' must expose 2-D data.joint_pos; "
                f"got {type(joint_pos)!r} shape={getattr(joint_pos, 'shape', None)}"
            )
        if not isinstance(joint_vel, torch.Tensor) or joint_vel.shape != joint_pos.shape:
            raise ValueError(
                f"Joint-state entity '{self.joint_state_entity}' must expose data.joint_vel "
                f"with shape {tuple(joint_pos.shape)}; got "
                f"{type(joint_vel)!r} shape={getattr(joint_vel, 'shape', None)}"
            )

        joint_count = int(joint_pos.shape[1])
        resolved: list[int] = []
        for index in self.joint_state_indices:
            resolved_index = index if index >= 0 else joint_count + index
            if resolved_index < 0 or resolved_index >= joint_count:
                raise IndexError(
                    f"Joint index {index} is invalid for entity '{self.joint_state_entity}' "
                    f"with {joint_count} joints"
                )
            resolved.append(resolved_index)
        self._resolved_joint_state_indices = tuple(resolved)

    def _read_joint_states(self, env_ids: Iterable[int]) -> dict[int, dict[str, np.ndarray]]:
        """Read actual joint position and velocity pairs for each requested env."""
        selected_ids = tuple(int(env_id) for env_id in env_ids)
        states_by_env: dict[int, dict[str, np.ndarray]] = {
            env_id: {} for env_id in selected_ids
        }
        if not selected_ids or self.joint_state_entity is None:
            return states_by_env
        if self._scene is None:
            raise RuntimeError("Scene is not initialized")
        if not self._resolved_joint_state_indices:
            self._resolve_joint_state_indices()

        data = getattr(self._scene[self.joint_state_entity], "data", None)
        joint_pos = data.joint_pos
        joint_vel = data.joint_vel
        indices = joint_pos.new_tensor(self._resolved_joint_state_indices, dtype=torch.long)
        env_indices = joint_pos.new_tensor(selected_ids, dtype=torch.long)
        selected_pos = joint_pos.index_select(1, indices).index_select(0, env_indices)
        selected_vel = joint_vel.index_select(1, indices).index_select(0, env_indices)
        pos_np = selected_pos.detach().cpu().numpy()
        vel_np = selected_vel.detach().cpu().numpy()
        for local_idx, env_id in enumerate(selected_ids):
            for joint_idx, label in enumerate(self.joint_state_labels):
                states_by_env[env_id][label] = np.asarray(
                    (pos_np[local_idx, joint_idx], vel_np[local_idx, joint_idx]),
                    dtype=float,
                )
        return states_by_env
    ...
    def _finalize_episode(
        ...
        *,
        ...
        if self.archive_every_episodes > 0 and episode % self.archive_every_episodes == 0:
            archive_file = self.archive_dir / f"env_{env_id:03d}" / f"episode_{episode:06d}.csv"
            self._atomic_write_csv(archive_file, rows)
    ...
    def _write_metadata(self) -> None:
        ...
        metadata = {
            ...
            "contact_sensors": [
                {
                    "alias": alias,
                    "name": sensor_name,
                    "prefix": self._contact_force_prefixes[alias],
                    "vector_frame": "world",
                    "force_units": "N",
                    "aggregation": "strongest filtered contact by vector magnitude",
                }
                for alias, sensor_name in self.contact_sensors.items()
            ],
            "joint_state": (
                {
                    "entity": self.joint_state_entity,
                    "indices": list(self._resolved_joint_state_indices),
                    "labels": list(self.joint_state_labels),
                    "prefix": self.joint_state_prefix,
                    "position_units": "rad",
            ...
            "archive_every_episodes": self.archive_every_episodes,
    ...
    def _build_columns(self) -> list[str]:
        ...
        for label in self.joint_state_labels:
            prefix = f"{self.joint_state_prefix}_{label}"
            columns.extend([f"{prefix}_pos_rad", f"{prefix}_vel_rad_s"])
        if self.include_actions:
